...
async def food_start(message: types.Message, state: FSMContext):
    ...
    await state.set_state(OrderFood.food_name.state)
...
async def food_chosen(message: types.Message, state: FSMContext):
    ...
    for size in available_food_sizes:
        keyboard.add(size)
    await state.set_state(OrderFood.food_size.state)
...
def register_handlers_food(dp: Dispatcher):
    ...
    dp.register_message_handler(food_chosen, state=OrderFood.food_name)
    dp.register_message_handler(food_size_chosen, state=OrderFood.food_size)
